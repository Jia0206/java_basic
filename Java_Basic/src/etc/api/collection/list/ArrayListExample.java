...
public class ArrayListExample {
	...
	public static void main(String[] args) {
        ...
        */
		...
		// *Arraylist()는 아무 객체가 다 담긴다.
		// *제네릭을 지정함으로써 한정됨
		// *new ArrayList<String>(); String 생략 가능
				
		String str = "야옹이";
		
		//*String str = "야옹이" 이 문장을 List<String>에 넣고
		//* 싶을 때
		...
		//String name = nick[3];(x) -> 배열의 방법이기 때문에 불가능
		//우리는 배열이 아닌 리스트를 배우는 것 
		...
		//*boolean 처럼 true false로 존재 유무만 밝혀줌 
		System.out.println(nick.contains("멍멍이"));
		
		//indexOf :특정 객체의 인덱스 탐색 (*인덱스 탐색해주는 메서드) 
		int idx = nick.indexOf("어흥이");
		System.out.println("어흥이의 인덱스: " + idx);
		//*인덱스 안에 없는 것은 -1로 나옴  ex) idx = -1
		...
		/*
		  # 컬렉션 객체들은 Collections 기능을 사용할 수 있습니다.
		  Collection는 컬렉션 객체들의 부가 기능들을 지원합니다.
		 */
		
		List<Integer> score = new ArrayList<>();
		//* 리스트는 인트를 받을 수 없음
		//* 정수를 저장할 때는 Integer을 사용해야 함
		score.add(65);
...
//		System.out.println(score);
		...
		//reverse는 정렬을 해주는 것이 아닌 오름차 정렬 먼저 한 후 
		// 내림차 정렬이 되는 것이다 
		// 오름차 정렬이 먼저 발생시켜야 진행됨 
